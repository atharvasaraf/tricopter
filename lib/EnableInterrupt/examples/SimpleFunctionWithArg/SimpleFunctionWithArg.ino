...
const uint8_t playful_id=ARDUINOPIN;
volatile uint16_t interruptCount=0; // The count will go back to 0 after hitting 65535.
volatile uint8_t latest_id=0;

void interruptFunction(uint8_t *id) {
  interruptCount++;
  latest_id=*id;
}
...
void setup() {
  ...
  enableInterrupt(ARDUINOPIN, interruptFunction((void *) &playful_id), CHANGE);
}
